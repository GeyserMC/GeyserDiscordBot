...
exports.populateInitialJiraVersions = async () => {
  const { status, data: versions } = await Utils.getContents('https://bugs.mojang.com/rest/api/latest/project/MC/versions')

  // Make sure we got a response
  if (versions === '' || status !== 200) {
    console.log('Failed to load initial Java jira versions')
    return
  }
