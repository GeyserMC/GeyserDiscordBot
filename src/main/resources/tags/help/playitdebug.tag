...
The most common issue when setting up playit.gg is confusing the Bedrock port and the playit.gg port, which are entirely separate. To fix this, change the Bedrock port  in your Geyser config back to the default of `19132`, and ensure that `clone-remote-port` is set to `false`.
...
- You have the program agent open when trying to join.
