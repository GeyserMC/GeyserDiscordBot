...
You can change the prefix in players' names by editing the option in Floodgate's config.yml file. Some characters that will not cause conflicts are `.` (default), `+`, `-`, and `!`.
**Using "*" as a prefix while on Windows will cause issues with some plugins**
